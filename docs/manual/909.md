...
|-----------|---------------|-------------------------------------------------------|
| 36        | Bass Drum     | TwinTBridge + pitch envelope sweep, saturation, VCA    |
...
The 909 BD uses a bridged-T resonant network that behaves as a damped sine resonator. A fast exponential pitch envelope sweeps from about 210 Hz at the attack down to the body frequency, about 55 Hz, in about 20 ms, producing the characteristic "thump". The amplitude envelope controls overall volume. A Tanh stage clips the output as the original's output amplifier does:

```
NoteGate → pitchEnv.gate    → Scale(55-210 Hz) → TwinTBridge.frequency
NoteGate → TwinTBridge.trigger
NoteGate → ampEnv.gate      → VCA.cv
TwinTBridge → Tanh(gain=5) → VCA → Level
```
...
A circuit with 23 parameters across nine instruments is difficult to navigate as a flat list. The `val:section` property on `val:Param` bindings groups parameters under named headings in the Knobs panel:
...
The Knobs panel renders a coloured label and a divider line at the start of each new section. Bindings without a `val:section` are displayed ungrouped. Each instrument now exposes a Level parameter as a `val:Gain` trim before the main mixer.
